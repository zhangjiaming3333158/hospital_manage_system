<template>
  <div class="editDoctor">
    ...
    <el-table :data="doctorlist" border style="width: 100%;">
      ...
      <el-table-column fixed prop="sex" label="性别" width="50">
      </el-table-column>
      <el-table-column fixed label="生日" width="150">
        <template scope="scope">
          {{scope.row.date}}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      intendedSearch.$http
        .get(api.doctor, { params: { name: intendedSearch.searchName } })
        .then(
          (response) => {
            ...
            for (let i = 0; i < response.data.doctorlist.length; i++) {
              if (
                response.data.doctorlist[i].name === intendedSearch.searchName
              ) {
